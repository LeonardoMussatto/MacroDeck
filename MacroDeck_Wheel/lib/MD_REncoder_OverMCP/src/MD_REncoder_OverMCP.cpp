...
MD_REncoder_MCP::MD_REncoder_MCP(uint8_t pinA, uint8_t pinB) : _pinA(pinA), _pinB(pinB), _state(R_START)
#if ENABLE_SPEED
                                                               ,
...
void MD_REncoder_MCP::begin(Adafruit_MCP23X08 &mcp)
{
  mcp.pinMode(_pinA, INPUT);
  mcp.pinMode(_pinB, INPUT);
  mcp.setupInterruptPin(_pinA, CHANGE);
  mcp.setupInterruptPin(_pinA, CHANGE);
}
...
{
  uint8_t pinState = (mcp.digitalRead(_pinB) << 1) | mcp.digitalRead(_pinA);

  _state = table[_state & 0xf][pinState];
